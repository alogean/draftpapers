...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private dtdslLexer lexer;
	private dtdslParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public dtdslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new dtdslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public dtdslParser getParser() {
		if (parser == null) {
			parser = new dtdslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "com::innoq::dtdsl::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"com::innoq::dtdsl::GenChecks","com::innoq::dtdsl::Checks"};
